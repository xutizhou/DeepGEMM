...
sm120_fp8_fp4_gemm_1d1d_impl(cd_dtype_t* gmem_d, const cd_dtype_t* gmem_c,
    ...
    static constexpr bool kSchedulerMulticastOnA = not kUseG1LegacyPath;
    static constexpr uint32_t kDefaultSchedulerGroup =
        sched::get_num_1d_blocks_per_group<kGemmType, BLOCK_M, BLOCK_N, kNumSMs, kSchedulerMulticastOnA>();
    static constexpr uint32_t kSchedulerGroup = kUseG1LegacyPath ? 11 : (kUseG2KFCandidatePath ? kDefaultSchedulerGroup : 7);
    auto scheduler = sched::Scheduler<kGemmType, BLOCK_M, BLOCK_N, kNumGroups, 1, kSchedulerMulticastOnA, kNumSMs, kSFKAlignment, kSchedulerGroup>(
